...
vector<Point2f> detectCorners(Mat &src)
{
    vector<Vec2f> lines;
...
Mat warpCard(Mat src)
{
    ...
    auto cornors = detectCorners(convexPoly);
...
vector<Rect> findBoundingRect(Mat src)
{
    ...
    rectangle(sobel, rects[2], Scalar(100, 212, 100),2);
    //imshow("desired rectangle", sobel);
...
vector<int> detectDigits(Mat gray, vector<Mat> digitROI)
{
    ...
    for (size_t i=0; i<rects.size(); i++)
    {
        ...
        drawContours(roi[i], digitCnt, 0, (0, 0, 255), 3);
        //imshow("ROI", roi[i]);
...
int main(int argc, char* argv[])
{
    ...
    string sample = file + "sample3.jpg";
    string font = file + "font.png";
    Mat src = imread(sample);
    resize(src, src, Size(600, 600.0*src.rows/src.cols));
    cvtColor(src, src, CV_BGR2GRAY);
   // imshow("resize+gray", src);
    auto warpedCard = warpCard(src);
   // imshow("warp", warpedCard);
    auto ref = getRefOCR(font);
